...
class Beam:
    def __init__(self, bird: Bird):
        ...
        self.img = pg.transform.rotozoom(self.img, theta, 1.0)
...
class Score:
    def __init__(self):
        ...
        self.score = 0
        self.img = self.font.render(f"スコア：", 0, self.font_color)
        self.rct = self.img.get_rect()
        self.rct.center = (100, HEIGHT - 50)

    def update(self, screen: pg.Surface):
        self.img = self.font.render(f"スコア：{self.score}", 0, self.font_color)
        screen.blit(self.img, self.rct)
    ...
    while True:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                return
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE:  #スペースキーが押されたらビームを生成
        ...
        if beams is not None:
            ...
            beams = [beam for beam in beams if (WIDTH > beam.rct.centerx > 0 and HEIGHT > beam.rct.centery > 0)]
